...
namespace KO
{
    ...
    public class KOFXRenderer : MonoBehaviour
    {
        ...
        public Texture2D LoadTexture(string relPath)
        {
            ...
            if (tex == null)
            {
                string cleanPath = relPath.Replace(".dxt", "").Replace(".DXT", "").Replace('\\', '/');
                ...
                string[] relSearchPaths = {
                    ...
                    $"KOTextures/fx/{cleanPath.ToLowerInvariant()}",
                    $"KOTextures/fx/particle/{fileBase}",
                    $"KOTextures/fx/particle/{fileBase.ToLowerInvariant()}",
                    $"KOTextures/fx/billboard/{fileBase}",
                ...
                if (tex == null && !string.IsNullOrEmpty(dirPart))
                {
                    string grp = fileBase;
                    if (grp.Length > 4 && int.TryParse(grp.Substring(grp.Length - 4), out _))
                        grp = grp.Substring(0, grp.Length - 4);

                    if (!dirPart.EndsWith(grp, System.StringComparison.OrdinalIgnoreCase))
                    {
                        tex = Resources.Load<Texture2D>($"KOTextures/{dirPart}/{grp}/{fileBase}")
                           ?? Resources.Load<Texture2D>($"KOTextures/{dirPart.ToLowerInvariant()}/{grp}/{fileBase}");
                    }
                }
            }
            ...
            _texCache[relPath] = tex; // Cache result (or null) to prevent 60fps log spam
